...
struct Expr_Unary : public Expr {
    ...
    explicit Expr_Unary(const EUnaryOperator op, ExprPtr inner) : op(op), inner(std::move(inner)) {}
};
...
struct Expr_Binary : public Expr {
    ...
    explicit Expr_Binary(ExprPtr left, const EBinaryOperator op, ExprPtr right) : left(std::move(left)), op(op), right(std::move(right)) {}
};
...
inline ExprPtr mock_parsed() {
    ...
    auto expr =  make_unique<Expr_Binary>(
        make_unique<Expr_Unary>(
            Expr_Unary::EUnaryOperator::Minus,
